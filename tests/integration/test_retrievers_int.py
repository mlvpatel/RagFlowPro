...
def test_hybrid_retrieval_surfaces_the_matching_document(pg_available):
    ...
    store.add_documents(
        [
            ...
            Document(page_content="quantum computing uses qubits", metadata={"file_id": 2}),
            Document(page_content="the dog ran across the park", metadata={"file_id": 3}),
        ]
    )
    ...
    assert "cat" in out[0].page_content, "the sparse match should rank the cat document first"
