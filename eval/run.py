...
def main(k: int = None) -> dict:
    ...
    _cleanup()
    store.add_documents(
        [Document(page_content=text, metadata={"file_id": fid}) for fid, text in DOCUMENTS.items()]
    )
    ...
    print(f"  Top-1 accuracy: {results['top1_accuracy']:.3f}   (top result is the right document)")
    print(f"  Hit@{k}:        {results['hit_rate']:.3f}   (right document in the top {k})")
    print(f"  MRR:            {results['mrr']:.3f}")
    print(f"  Recall@{k}:     {results['recall']:.3f}")
    print(f"  Precision@{k}:  {results['precision']:.3f}   (bounded by one relevant doc over {k})")
